...
async function VoiceChatCreate(interaction) {
    logger.trace("[button] VoiceChatCreate.js");
    ...
    switch(interaction.channelId){//ボタンのあるチャンネルID
        case '1108624508211966012'://debug鯖でのテスト用
            logger.trace("[button] VoiceChatCreate.js: debug テスト");
            const VC_name_test = "テスト",category_test = '1108624382009548810',bitrate_test = 96000;
            await execute(interaction, category_test, VC_name_test, bitrate_test);
            break;
        case '1108678708480446535'://CASINO サブVC
            logger.trace("[button] VoiceChatCreate.js: CASINO サブVC");
            const VC_name_sub = "サブ",category_sub = '1108678500963061891',bitrate_sub = 192000;
...
function execute(interaction, category, VCname, VCbitrate) {
    logger.trace("[button] VoiceChatCreate.js: execute");
