const { ChatInputCommandInteraction, Client, ActionRowBuilder, EmbedBuilder, PermissionFlagsBits } = require('discord.js');
...
async function create(interaction, client) {
	...
	if(!(interaction.member.permissions.has(PermissionFlagsBits.ManageMessages) || interaction.member.permissionsIn(channel).has(PermissionFlagsBits.ManageMessages))) {//メッセージの管理権限を持っていない場合
		interaction.followUp({content: 'タイマーの作成に必要な権限がないため作成出来ませんでした。権限を持つユーザーにお問い合わせください。', ephemeral: true});
		return;
	}
	...
	const res = await dbclient.connection(`INSERT INTO visual_timer (channel_id, message_id, author_id, name, description, time) VALUES ('${channel.id}', '${timer_message.id}', '${User.id}', '${name}', '${description}', ${time})`);
